...
export function PipeScheduleBuilder({
  ...
  const t2 = useTranslations("scheduleUi5");
  ...
  return (
    <div className="w-[320px] space-y-3 p-1 text-xs">
      ...
      <div className="flex items-center justify-between gap-2">
        <Label className="text-xs">{t("runOnSchedule")}</Label>
      ...
      {!manual && (
        <>
          {/* Repeat */}
          <div className="flex items-center justify-between gap-2">
            <Label className="text-xs">{t("repeat")}</Label>
            <Select
              value={cfg.frequency}
              ...
              <SelectContent>
                {FREQUENCY_OPTIONS.map((f) => (
                  <SelectItem key={f.value} value={f.value}>
                    {t("every", { value: f.label.replace(/s$/, "") })}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {/* Every N <unit> */}
          <div className="flex items-center justify-between gap-2">
            <Label className="text-xs">{t("everyLabel")}</Label>
          ...
          {cfg.frequency === "weeks" && (
            <div>
              <Label className="text-xs mb-1 block">{t("on")}</Label>
              ...
              {weeklyNoDays && (
                <p className="mt-1 text-[11px] text-destructive">{t("pickDay")}</p>
              )}
            </div>
          )}
          ...
          {cfg.frequency === "months" && (
            <div className="flex items-center justify-between gap-2">
              <Label className="text-xs">{t("onThe")}</Label>
              <Select
          ...
          {showTime && (
            <div className="flex items-center justify-between gap-2">
              <Label className="text-xs">{t("at")}</Label>
              <input
                type="time"
                aria-label={t("time")}
                value={timeValue}
          ...
          {/* Starting / Ending */}
          <div className="flex items-center justify-between gap-2">
            <Label className="text-xs">{t("starting")}</Label>
            <input
              type="date"
          ...
          <div className="flex items-center justify-between gap-2">
            <Label className="text-xs">{t("ending")}</Label>
            <Select
              value={endMode}
              onValueChange={(v) => {
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="on">{t("onDate")}</SelectItem>
                <SelectItem value="after">{t("afterRuns")}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          {endMode === "on" && (
      ...
      <div className="rounded border border-border bg-muted/30 px-2 py-1.5">
        {manual ? (
          ...
          <>
            <p className="text-[11px]">
              {t("runsSummary")} <span className="text-foreground">{summary}</span>
            </p>
            {nextRun && (
              <p className="text-[11px] text-muted-foreground">{t("nextOccurrence")}: {nextRun}</p>
            )}
          </>
        )}
      </div>
