...
export function TimelineTagToolbar({ anchorRect, onAskAI, onRunPipe, templatePipes }: TimelineTagToolbarProps) {
	...
	const u = useTranslations("rewindUi");
	const tu = useTranslations("timelineUi2");
	const tu3 = useTranslations("timelineUi3");
	const tu5 = useTranslations("timelineUi5");
	...
	const handleApplyTag = useCallback(async (tag: string) => {
		...
		try {
			...
			toast({
				title: u("tagged"),
				description: tu3("tagApplied", { tag: tag.trim(), count: frameIds.length }),
			});
			...
			toast({
				title: u("taggingFailed"),
				description: u("someFramesFailed"),
				variant: "destructive",
			});
		} finally {
			setIsApplying(false);
	...
	const handleRemoveTag = useCallback(async (tag: string) => {
		...
		try {
			...
			toast({
				title: u("removeFailed"),
				description: tu3("removeTagFailed", { tag }),
				variant: "destructive",
			});
		} finally {
			setIsApplying(false);
	...
	const handleDeleteRange = useCallback(async () => {
		...
		try {
			...
			toast({
				title: u("deleted"),
				description: tu3("rangeDeleted", { frames: data.frames_deleted, audio: data.audio_transcriptions_deleted }),
			});
			...
			toast({ title: u("deletionFailed"), description: String(e), variant: "destructive" });
		} finally {
			setIsDeleting(false);
		}
				...
				{showDeleteConfirm ? (
					...
					<div className="flex flex-col gap-2">
						<p className="text-xs text-muted-foreground leading-relaxed">
							{tu5("deleteRangePrefix")} {" "}
							<span className="font-medium text-foreground">{startTime} → {endTime}</span> ({durationStr}){tu5("cannotUndo")}
						</p>
