        ...
        {errorText && (
          <div className="mb-8 border border-border px-4 py-3 flex items-center justify-between gap-3">
            <div className="text-xs">
              <div className="text-foreground">{t("loadFailed")}</div>
        ...
        {trulyEmpty && !errorText ? (
          ...
          isSearchActive ? (
            <p className="text-sm text-muted-foreground py-8">
              {searching
                ? t("searching")
                : t("noMatchingMeetings", { query: searchInput.trim() })}
            </p>
          ) : (
...
function RecordingStrip({
  ...
  const statusLabel = captureState ? ct(`${captureState.kind}_short`) : ct("recording_short");
